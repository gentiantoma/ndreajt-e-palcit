...
const { getFirestore, collection, addDoc, getDocs, query, where, Timestamp }
  = require('../node_modules/firebase/firestore/dist/index.cjs.js');
...
const DEMO_POSTS = [
  {
    ...
    category: 'lajme',
    authorId: 'seed-admin', authorName: 'Gentian Toma', authorPhoto: '',
    coverImage: 'https://picsum.photos/seed/palc-village1/900/500',
    images: [],
    likeCount: 47, commentCount: 12, published: true,
    createdAt: ts(2 * 3600 * 1000), updatedAt: ts(2 * 3600 * 1000),
  },
  {
    titleSq: 'Festa e Vjeshtës — Palç 2025',
    titleEn: 'Autumn Festival — Palç 2025',
    bodySq: 'Sivjet festa e vjeshtës ishte diçka e paharrueshme! Banorë nga e gjithë diaspora u mblodhën bashkë për të festuar traditat tona. Muzikë live, ushqime tradicionale dhe valle shqiptare deri në mesnatë. Faleminderit të gjithëve!',
    ...
    likeCount: 124, commentCount: 38, published: true,
    createdAt: ts(24 * 3600 * 1000), updatedAt: ts(24 * 3600 * 1000),
  },
  {
    ...
    images: [],
    likeCount: 89, commentCount: 21, published: true,
    createdAt: ts(3 * 24 * 3600 * 1000), updatedAt: ts(3 * 24 * 3600 * 1000),
  },
  {
    ...
    bodyEn: 'My grandmother, 87, tells how life was in Palç 60 years ago. "We had everything — fresh water, forests full of animals, neighbors like brothers."',
    category: 'histori',
    authorId: 'seed-admin', authorName: 'Gentian Toma', authorPhoto: '',
    coverImage: 'https://picsum.photos/seed/palc-history/900/500',
    images: ['https://picsum.photos/seed/palc-hist2/900/500'],
    likeCount: 203, commentCount: 54, published: true,
    createdAt: ts(5 * 24 * 3600 * 1000), updatedAt: ts(5 * 24 * 3600 * 1000),
  },
  {
    ...
    bodyEn: 'Few places in the world have such pristine nature. These photos were taken during the morning walk near the river.',
    category: 'lajme',
    authorId: 'seed-admin', authorName: 'Gentian Toma', authorPhoto: '',
    ...
    likeCount: 156, commentCount: 29, published: true,
    createdAt: ts(7 * 24 * 3600 * 1000), updatedAt: ts(7 * 24 * 3600 * 1000),
  },
];
...
async function seed() {
  ...
  try {
    ...
    // Firestore might have permission rules — proceed anyway
    console.warn('Could not check existing posts, proceeding with seed...');
  }

  console.log('🌱 Seeding Firestore...\n');
  for (const post of DEMO_POSTS) {
    const ref = await addDoc(collection(db, 'posts'), post);
    console.log(`  ✅  "${post.titleSq}"  →  ${ref.id}`);
  }
  console.log(`\n🎉 Done! ${DEMO_POSTS.length} posts seeded.`);
  process.exit(0);
}

seed().catch(err => {
  console.error('\n❌ Seed failed:', err.message);
  console.error('\nMake sure Firestore rules allow writes (test mode) in the Firebase console.');
  process.exit(1);
});
